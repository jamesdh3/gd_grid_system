...
public class HeatMapBoolVisual : MonoBehaviour {
    ...
    private void Awake() {
        mesh = new Mesh();
        GetCompononent<MeshFilter>().mesh = mesh; 
    }
    
    private void SetGrid(Grid<bool> grid) {
        this.grid = grid;
        UpdateHeatMapVisual();
    ...
    private void UpdateHeatMapVisual() {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                // MeshUtils.AddToMeshArrays(vertices, uv, triangles, index, grid.GetWorldPosition(x,y) + quadSize * .5f, 0f, quadSize, gridValueUV, 
            }
        }
